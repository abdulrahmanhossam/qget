...
func main() {
	...
	go func() {
		<-sigChan
		fmt.Println("Interrupted by user. Exiting gracefully...")
		os.Exit(0)
	}()

	outputDir := flag.String("o", ".", "Output directory for the downloaded file")
	updateDeps := flag.Bool("update", false, "Update yt-dlp to the latest version")
	flag.Parse()
	...
	if *updateDeps {
		...
		ytDlpPath := filepath.Join(appDir, binaryName)
		...
		fmt.Println("==> Updating underlying yt-dlp dependencies...")
	...
	if isPlaylist {
		...
		if confirm {
			...
			if formatType == "audio" {
				...
				fmt.Println("Fetching available resolutions...")
				resolutions, err := video.GetAvailableResolutions(url, ytDlpPath, denoPath, true)
				if err != nil {
					fmt.Printf("Failed to get resolutions: %v\n", err)
					os.Exit(1)
				}

				quality, err := ui.SelectQuality(resolutions)
				if err != nil {
	...
	fmt.Println("Fetching available resolutions...")
	resolutions, err := video.GetAvailableResolutions(url, ytDlpPath, denoPath, false)
	if err != nil {
		fmt.Printf("Failed to get resolutions: %v\n", err)
		os.Exit(1)
	}
